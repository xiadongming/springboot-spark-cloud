...
public class RedisServiceImpl implements IRedisService {
    ...
    public List<Coupon> getCachedCoupons(Long userId, Integer status) {
        ...
        System.out.println("Get Coupons From Cache: {}, {}" + userId + " , " + status);
    ...
    public void saveEmptyCouponListToCache(Long userId, List<Integer> status) {
        ...
        System.out.println("Save Empty List To Cache For User: {}, Status: {}" + userId + "," + JSON.toJSONString(status));

        // key 是 coupon_id, value 是序列化的 Coupon
        Map<String, String> invalidCouponMap = new HashMap<>();
        invalidCouponMap.put("-1", JSON.toJSONString("12121"));
        ...
        String result = JSON.toJSONString(redisTemplate.executePipelined(sessionCallback));
        System.out.println("Pipeline Exe Result: {}" + result);
    }
    ...
    public String tryToAcquireCouponCodeFromCache(Integer templateId) {
        ...
        System.out.println("Acquire Coupon Code: {}, {}, {}" + templateId + "" + redisKey + "" + couponCode);
     ...
     */
    @Override
    public Integer addCouponToCache(Long userId, List<Coupon> coupons, Integer status) throws Exception {

        System.out.println("Add Coupon To Cache: {}, {}, {}" + userId + "," + JSON.toJSONString(coupons) + "," + status);
    ...
    private Integer addCouponToCacheForUsable(Long userId, List<Coupon> coupons) {
        ...
        System.out.println("Add Coupon To Cache For Usable.");
        ...
        System.out.println("Add {} Coupons To Cache: {}, {}" + needCachedObject.size() + "," + userId + "," + redisKey);
    ...
    private Integer addCouponToCacheForUsed(Long userId, List<Coupon> coupons) throws Exception {
        // 如果 status 是 USED, 代表用户操作是使用当前的优惠券, 影响到两个 Cache
        // USABLE, USED
        System.out.println("Add Coupon To Cache For Used.");
        ...
        if (!CollectionUtils.isSubCollection(paramIds, curUsableIds)) {
            System.out.println("CurCoupons Is Not Equal ToCache: {}, {}, {}" + userId + "," + JSON.toJSONString(curUsableIds) + "," + JSON.toJSONString(paramIds));
            throw new Exception("CurCoupons Is Not Equal To Cache!");
        }
        ...
        System.out.println("Pipeline Exe Result: {}" + JSON.toJSONString(redisTemplate.executePipelined(sessionCallback)));
        return coupons.size();
    }
     ...
     */
    private Integer addCouponToCacheForExpired(Long userId, List<Coupon> coupons) throws Exception {

        // status 是 EXPIRED, 代表是已有的优惠券过期了, 影响到两个 Cache
        // USABLE, EXPIRED

        System.out.println("Add Coupon To Cache For Expired.");
        // 最终需要保存的 Cache
        ...
        List<Integer> paramIds = coupons.stream().map(Coupon::getId).collect(Collectors.toList());
        if (!CollectionUtils.isSubCollection(paramIds, curUsableIds)) {
            System.out.println("CurCoupons Is Not Equal To Cache: {}, {}, {}" + userId + "," + JSON.toJSONString(curUsableIds) + "," + JSON.toJSONString(paramIds));
            throw new Exception("CurCoupon Is Not Equal To Cache.");
        }
